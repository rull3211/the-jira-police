...
Status: running end to end against production Jira. 2495 tests in 69 files, no build step, no
...
- **`TRIAGE_ONLY_STATUS` set** → `status IN (…)`. The default names this board's untouched columns.
...
the eligible set straddles the taxonomy and the per-board strings have to be named after all. In the
component scope this narrowed 106 eligible tickets to 14.

**Nothing validates those strings**, which is the cost of naming them: a typo or a renamed column is
a filter matching nothing, and a service that looks healthy while triaging zero tickets forever. The
credential cannot read project status metadata to check them (`/project/SSX/statuses` is a 404 for
it), so the mitigation is visibility rather than validation — `poll.status_filter` prints the active
list once at wiring, at `info`, unlike the per-cycle `poll.query` at `debug`.
...
| `TRIAGE_ONLY_STATUS`            | `Mottatt,Backlog,On Hold,In Progress Concept` | Statuses discovery may triage. Blank means anything not closed. See §4                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
