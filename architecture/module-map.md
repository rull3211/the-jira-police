...
| `src/solve/dependency-bump.ts` | Whether a changed `pom.xml` is only dependency version bumps and comment edits: the diff gate's one exception, asked by the gate and `verify` alike     |
