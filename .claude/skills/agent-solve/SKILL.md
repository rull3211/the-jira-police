...
- **Stay inside the stated scope.** The harness refuses a list of paths outright — including
  anything that would change what "passing" means — and those refusals discard the whole run, or
  in a review round at least the refused edit. It
...
- **Add no dependencies.** Not to the manifest, not to a lockfile. If the task cannot be done with
  ...
  already declares is not a new dependency**: change that version — or, in the repository's only
  `pom.xml`, with no parent or modules, the property only it uses — and nothing else in the file
  but the text of a comment already in it, and the harness allows it, naming the version in the
  pull request. A
