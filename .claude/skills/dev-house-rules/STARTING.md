...
`pnpm test:hooks`, run it if you change one — but whether anything registers them is decided outside
this tree and is unreadable from inside it, and the second rule has no guard here at all
(`PLAN.md` §12). Assume neither is watched. Neither is any less binding for that; the only
difference is that breaking one may not be caught.
