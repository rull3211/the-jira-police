...
2. **`PLAN.md` records what is not built**, gaining an entry when work _starts_ and losing it when
   the work ships — **deleted**, not struck through, unless it is a lesson that lives nowhere else
   and moves to the "learned" section. A plan that accumulates finished work stops being read, and a
   plan nobody reads is how two people build the same thing twice.
