...
describe("formatSolveReport", () => {
  ...
  it("admits when the deps were not composed from settings", () => {
    // Built literally, not via the helper with `undefined`: `exactOptionalPropertyTypes`
    // is on, and the key under test must be genuinely absent.
...
describe("decisionLines", () => {
  ...
  it("prints only the named ticket, and none of its neighbours", () => {
    // A leaky filter would print another ticket's claim under the key the operator typed.
