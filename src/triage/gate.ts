...
/**
 * The line that opens the agent-fitness block, and the marker that makes the
 * block strippable.
 *
 * Lives here beside the footer sentinel because it is the same kind of thing
 * and for the same reason: *our own activity is excluded by kind, never by
 * clock*, and both are the sentinel the writer controls. The difference is what
 * each one is for — the footer finds the comment, this finds a region inside
 * it.
 *
 * A prefix rather than a whole line, because what follows it varies with the
 * verdict, the repo and the confidence, and a model re-typing the block varies
 * it further: `🤖 **Agent fitness: solvable**` was observed on SSX-3024 where
 * the renderer writes `🤖 **Agent fitness:** looks automatable`. Matching to the
 * end of `fitness` catches both and stops before anything that moves.
 */
...
function checkComment(payload: TriagePayload, issueKey: string): readonly string[] {
  ...
  if (markers > 1) {
    // `withFitnessNote` strips before it splices, so reaching here means the
    // strip missed a paraphrase of the marker it is keyed on. Refusing is the
    // right end of that: a silent second block is the prose-versus-field
    // disagreement the block is rendered in TypeScript to prevent, and it is
    // nearly invisible on a ticket because both copies look official.
    violations.push(
