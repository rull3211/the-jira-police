...
export const SETTINGS = [
  ...
  },
  {
    name: "REPAIR_ROUND",
    description:
      "Whether a solve or a review round whose verification failed also runs one repair pass, which is shown the harness's captured failure and tries to correct the diff. On by default, the same shape as FAIL_FIRST_CHECK — but read the difference before copying the reasoning: that one writes nothing, and this one runs a write pass. It arms a model session holding Write and Edit, in a worktree the run is already writing, and buys a re-verification and sometimes a fail-first probe. What it does not do on its own is let anything act on the result. The outcome stays `failed` whatever the round concludes unless the run was also armed — `--repair` typed by hand, or REPAIR_PUBLISH for the daemon — because a green re-verification is reachable by deleting the failing assertion, which no exit code tells from a fix. Those are the half of the privilege that fails closed, so this one stays the mirror of `flag` even now that a verdict can be acted on: the thing a typo here would withdraw is still the measurement, not a guard on the writing — concretely, rows stop appearing in repair-rounds.md and `pnpm repair:ledger` has nothing new to read. A setting that let a round act with nothing typed would be a privilege switch and belong on `flag`; this is not that setting — REPAIR_PUBLISH is. Set it to false for cost.",
    fallback: "true",
  },
  {
    name: "REPAIR_PUBLISH",
    description:
      "Whether the daemon may act on a repair round that turned a failed verification green: open the pull request from it after a solve, or push it to the pull request already open after a review round — the loop's copy of `--repair`, which a person types per run and reads the result of. Off by default, and it is the switch that most needs to be: a repair is the one change this service opens that was written by a pass shown the red output, and a check made green by weakening the assertion that failed looks exactly like a fix. What a reviewer then sees is a draft pull request with every check passing and a plausible diff, which is the configuration in which review is weakest. The body, or for a review round a comment posted on the pull request, names the repair above everything else and it arrives as its own commit, but with this on nobody stands between the round and the pull request. Off, the daemon records every round in repair-rounds.md and discards the verdict, exactly as it did before this setting existed. Only `true` arms it, in any case, so a typo fails closed. Separate from REPAIR_ROUND rather than that one moved onto `flag`: REPAIR_ROUND decides whether a round is bought and defaults on because alone it only measures, while this decides whether an unattended round may act. The privilege is the two together, so REPAIR_PUBLISH=true with REPAIR_ROUND=false arms nothing, and the daemon says so at startup.",
    fallback: "false",
  },
  {
    name: "DEPENDENCY_BUMPS",
    description:
      "Whether a solve run may change a pom.xml when the only change is the version of a dependency the file already declares, the text of a comment, or both — the one exception to the rule that no run edits a file defining what passing means (architecture/solve.md §15). The pull request names every bump above anything a model wrote; a comment edit, which Maven never reads, gets no notice. On by default, by the operator's decision, against the house rule that a setting granting a privilege defaults off: bumping a library to get what a ticket needs is treated as ordinary work, and comment edits ride the same switch, as proposed to the operator on 2026-09-24 and accepted, since Maven discards comments. Read through `flag`, so only `true` arms it and a typo turns it off rather than on. Off, a plan naming pom.xml is stopped before the fix pass and any pom.xml change is refused at the diff gate, exactly as before the exception existed.",
    fallback: "true",
  },
  {
    name: "LOG_LEVEL",
