...
| **F**   | Sendback subscription (§7)                                                                                                      | re-triage spend with nobody asking                 | **built 2026-09-06, `feat/sendback-watch`.** 2134 tests. `watch:once --write` unsubscribes and re-triages; the attempt is reserved on a label before anything is paid for, and the check is shown what the edited fields now say. **Nothing loops yet** — no daemon cadence, and `WATCH_ENABLED` is still read by nothing |
