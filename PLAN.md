...
> **2811 tests in 92 files**, no build step.
>
...
**The holes are §4, §12, §15, §16, §18, §19, §20, §21, §22, §23, §25, §26, §27, §28, §29, §30, §32, §34, §35, §36, §37, §38, §40 and §41, and this line names them rather than
...
one. Its second use is the worktree rule that became rule 3 of `CLAUDE.md`, opened in `ed9ddb1`
against a hole line that **already named §40**, and closed in `803d2f4`. That closing commit is 78
deletions and no insertions: it removed the §38 and §40 entries without touching the list that was
supposed to gain them, and its message reasoned that "nothing else in the tree cited either number"
— true of every other file, and not of the paragraph it was editing. **A deletion-only commit cannot
update a list, and nothing here checks that it did**; the numbers were picked by reading the last
heading rather than this line, which is the reuse. §41 was the audit that found all of the above and
repaired it — opened and deleted inside its own branch, the shape §26, §27, §28, §30 and §32 have,
and the branch that closes it is the one that added the three names above and §41's own. The
...
**One level down, the split already exists for abandonment and only for abandonment**, which is
worth naming so it is not built twice: `abandonedCause` is `judgement` or `environment`, and
`solveWithRetry` retries once on `environment` and never on `judgement`, logging `solve.retry` when
it does. That is the shape this entry wants, applied to the one outcome that already carries a
cause. `refused` and `failed` carry none, which is why they still release — so the work is a cause
on those two verdicts before it is a policy about them.
